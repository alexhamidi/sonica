import { auth } from "@/lib/auth/server";
import { sql } from "@/lib/server/db";
import { NextResponse } from "next/server";

export const dynamic = "force-dynamic";

const SEARCHES_ID = "00000000-0000-0000-0000-000000000001";
const ORPHANS_ID = "00000000-0000-0000-0000-000000000002";

type ReprojectFlagRow = { reproject_pending: boolean };

export async function GET() {
  const { data: session } = await auth.getSession();
  if (!session?.user)
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const userId = session.user.id;

  try {
    // Idempotent — ensures new Google sign-ups (which bypass /sign-up/email) get
    // their default grandparent rows. No-op for existing users.
    await sql`
    INSERT INTO user_grandparents (user_id, grandparent_id)
    VALUES (${userId}, ${SEARCHES_ID}), (${userId}, ${ORPHANS_ID})
    ON CONFLICT DO NOTHING
  `;

    const [grandparentRows, entityRows, trackRows, reprojectRow] =
      await Promise.all([
        sql`
        SELECT gp.id, gp.name, gp.type, gp.cover_s3
        FROM user_grandparents ug
        JOIN grandparents gp ON gp.id = ug.grandparent_id
        WHERE ug.user_id = ${userId}
        ORDER BY CASE gp.type WHEN 'searches' THEN 0 WHEN 'orphans' THEN 2 ELSE 1 END, ug.added_at
    `,
        sql`
        SELECT p.id, p.name, p.cover_s3, p.type, p.status AS parent_status,
               p.query_kind AS query_kind,
               gp.id       AS grandparent_id,
               gp.name     AS grandparent_name,
               gp.cover_s3 AS grandparent_cover_s3,
               gp.type     AS grandparent_type,
               up.canvas_visible AS canvas_visible,
               (
                 SELECT COUNT(*)::int
                 FROM parent_tracks pt
                 JOIN tracks t ON t.id = pt.track_id AND t.status = 'ready' AND (t.is_query = false OR t.is_query IS NULL)
                 WHERE pt.parent_id = p.id
               ) AS track_count
        FROM user_parents up
        JOIN parents p ON p.id = up.parent_id
          AND EXISTS (SELECT 1 FROM parent_tracks pt0 WHERE pt0.parent_id = p.id)
        JOIN grandparents gp ON gp.id = p.grandparent_id
        JOIN user_grandparents ug
          ON ug.user_id = up.user_id AND ug.grandparent_id = gp.id
        WHERE up.user_id = ${userId}
        ORDER BY CASE gp.type WHEN 'searches' THEN 0 WHEN 'orphans' THEN 2 ELSE 1 END,
                 ug.added_at, up.added_at, p.created_at
    `,
        sql`
        SELECT
            t.id, t.title, t.artist, t.spotify_id, t.audio_s3, t.cover_s3, t.is_query,
            p.query_kind AS parent_query_kind,
            COALESCE(utp.umap_x, pt.umap_x) AS umap_x,
            COALESCE(utp.umap_y, pt.umap_y) AS umap_y,
            COALESCE(utp.pca_x,  pt.pca_x)  AS pca_x,
            COALESCE(utp.pca_y,  pt.pca_y)  AS pca_y,
            utp.tsne_x, utp.tsne_y,
            p.id AS source_parent_id,
            cat_art.cover_s3 AS catalog_artist_cover_s3
        FROM user_parents up
        JOIN parents p ON p.id = up.parent_id AND p.status = 'ready'
        JOIN parent_tracks pt ON pt.parent_id = p.id
        JOIN tracks t ON t.id = pt.track_id AND t.status = 'ready'
        LEFT JOIN LATERAL (
          SELECT COALESCE(gp.cover_s3, gc.cover_s3) AS cover_s3
          FROM parent_tracks pt_a
          JOIN parents p_art ON p_art.id = pt_a.parent_id AND p_art.status = 'ready'
          JOIN grandparents gp ON gp.id = p_art.grandparent_id
            AND gp.type = 'artist' AND gp.status = 'ready'
          LEFT JOIN (
            SELECT p2.grandparent_id, MIN(t2.cover_s3) AS cover_s3
            FROM parents p2
            JOIN parent_tracks pt2 ON pt2.parent_id = p2.id
            JOIN tracks t2 ON t2.id = pt2.track_id
            WHERE t2.cover_s3 IS NOT NULL
            GROUP BY p2.grandparent_id
          ) gc ON gc.grandparent_id = gp.id
          WHERE pt_a.track_id = t.id
          ORDER BY p_art.created_at ASC NULLS LAST
          LIMIT 1
        ) cat_art ON true
        LEFT JOIN user_track_projections utp
               ON utp.user_id = ${userId} AND utp.track_id = t.id
        WHERE up.user_id = ${userId}
        ORDER BY up.added_at, p.id, t.id
    `,
        sql`
        SELECT EXISTS (
          SELECT 1 FROM user_grandparents
          WHERE user_id = ${userId} AND projected = false
        ) AS reproject_pending
    `,
      ]);

    const S3_BASE = process.env.S3_BASE ?? "";
    const s3 = (key: string | null | undefined) => {
      if (!key) return null;
      if (key.startsWith("http://") || key.startsWith("https://")) return key;
      const k = key.startsWith("s3://")
        ? key.split("/").slice(3).join("/")
        : key;
      return `${S3_BASE}/${k}`;
    };

    const entities = entityRows.map((row) => ({
      id: String(row.id),
      name: row.name as string,
      cover: s3(row.cover_s3 as string | null),
      type: row.type as string,
      queryKind:
        row.query_kind != null && row.query_kind !== ""
          ? String(row.query_kind)
          : null,
      parentId: row.grandparent_id ? String(row.grandparent_id) : null,
      parentName: row.grandparent_name as string | null,
      parentCover: s3(row.grandparent_cover_s3 as string | null),
      parentType: row.grandparent_type as string | null,
      trackCount: Number(row.track_count ?? 0),
      canvasVisible: Boolean(row.canvas_visible ?? true),
      status: row.parent_status as string,
    }));
    const entityIndex = new Map(entities.map((e, i) => [e.id, i]));

    const tracks = trackRows.map((row, i) => {
      const projections: Record<string, [number, number]> = {};
      if (row.umap_x != null)
        projections.umap = [Number(row.umap_x), Number(row.umap_y)];
      if (row.pca_x != null)
        projections.pca = [Number(row.pca_x), Number(row.pca_y)];
      if (row.tsne_x != null)
        projections.tsne = [Number(row.tsne_x), Number(row.tsne_y)];
      return {
        index: i,
        trackId: String(row.id),
        title: row.title,
        artist: row.artist,
        spotifyId: row.spotify_id ? String(row.spotify_id) : null,
        mp3: s3(row.audio_s3 as string | null),
        cover: s3(row.cover_s3 as string | null),
        sourceIndex: entityIndex.get(String(row.source_parent_id)) ?? 0,
        sourceEntityId: String(row.source_parent_id),
        projections,
        isQuery: Boolean(row.is_query),
        sourceQueryKind:
          row.parent_query_kind != null && row.parent_query_kind !== ""
            ? String(row.parent_query_kind)
            : null,
        artistCover: s3(row.catalog_artist_cover_s3 as string | null),
      };
    });

    const grandparents = grandparentRows.map((row) => ({
      id: String(row.id),
      name: row.name as string,
      type: row.type as string,
      cover: s3(row.cover_s3 as string | null),
    }));

    const reprojectPending = Boolean(
      (reprojectRow[0] as ReprojectFlagRow | undefined)?.reproject_pending,
    );

    return NextResponse.json({
      grandparents,
      entities,
      tracks,
      reprojectPending,
    });
  } catch (e) {
    const detail = e instanceof Error ? e.message : String(e);
    console.error("[api/me/canvas]", e);
    return NextResponse.json(
      {
        error: "Canvas query failed",
        detail,
        hint: "If detail mentions an undefined column, run mus/py/scripts/ensure_api_schema.sql against POSTGRES_URL.",
      },
      { status: 500 },
    );
  }
}
